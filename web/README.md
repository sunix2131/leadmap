...
Задача из административной панели считается завершённой только после импорта в PostgreSQL. Импортируются CSV, созданные или изменённые этим запуском; старые выгрузки не перечитываются. Ошибка импорта откатывает его транзакцию и переводит задачу в состояние ошибки. Отмена останавливает дочерний процесс и не запускает импорт.

В одном процессе backend допускается одна активная задача: CLI использует общую папку `out/` и реестр. История ограничена 100 задачами. Запуск, вывод и отмена доступны только администратору. Используйте один worker backend; параллельные CLI-запуски вне CRM с этой папкой не поддерживаются.

Импорт понимает списки через `;` из Maps Parser, старые списки через запятую и JSON-массивы. Даты с часовым поясом приводятся к UTC перед записью в существующую схему PostgreSQL.
...
В CI также поднимается PostgreSQL 16: проверяются создание схемы с нуля, повторный импорт без дубликатов и откат всего запуска при ошибке строки. Локально этот тест включается через `TEST_DATABASE_URL`; имя отдельной тестовой базы должно оканчиваться на `_test`. Таблицы и данные теста создаются в откатываемой транзакции.
